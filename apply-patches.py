...
def librewolf_patches(firefox_folder: str, common_srcdir: str, settings_srcdir: str):
    ...
    exec('mkdir -p lw')
    enter_srcdir('lw')
    exec('cp -v {0} .'.format(join(settings_srcdir, 'gnoppix.cfg')))
    ...
    # 2) new files
    exec('cp {0} browser/themes/shared/preferences/category-gnoppix-browser.svg'.format(join(common_srcdir, 'patches/pref-pane/category-gnoppix-browser.svg')))
    exec('cp {0} browser/themes/shared/preferences/gnoppix-browser.css'.format(join(common_srcdir, 'patches/pref-pane/gnoppix-browser.css')))
    exec('cp {0} browser/components/preferences/gnoppix-browser.inc.xhtml'.format(join(common_srcdir, 'patches/pref-pane/gnoppix-browser.inc.xhtml')))
    exec('cp {0} browser/components/preferences/gnoppix-browser.js'.format(join(common_srcdir, 'patches/pref-pane/gnoppix-browser.js')))
...
if len(args) != 3:
    sys.stderr.write(f'Invalid usage! {sys.argv[0]} <firefox folder> <gnoppix-browser-common folder> <gnoppix-browser-settings folder>\n')
    sys.exit(1)

firefox_folder = args[0]
gnoppix_common_folder = args[1]
gnoppix_settings_folder = args[2]
...
gentoo_patches(firefox_folder, gnoppix_common_folder)

print('---- Librewolf patches')
librewolf_patches(firefox_folder, gnoppix_common_folder, gnoppix_settings_folder)
