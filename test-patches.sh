...
[[ -d "$SETTINGSDIRTEST" ]] || git clone --depth 1 -q "https://github.com/gnoppix/gnoppix-browser-common.git" "$SETTINGSDIRTEST"
...
# we keep that until we actually create locale for Firefox to replace strings with "Gnoppix"
echo ">>> add warning that sanitizing exceptions are bypassed by the options in History > Clear History when Gnoppix closes > Settings"
